...
EOF
  ...
  red "存储变量到文件${PWD}/${save_var_file}成功"
  ...
  if [ -e npm ]; then
    ...
    if [ -n "$NEZHA_SERVER" ] && [ -n "$NEZHA_PORT" ] && [ -n "$NEZHA_KEY" ]; then
      ...
      pgrep -x "npm" >/dev/null && green "npm is running" || {
        ...
        pkill -x "npm" && nohup ./npm -s "${NEZHA_SERVER}:${NEZHA_PORT}" -p "${NEZHA_KEY}" ${NEZHA_TLS} >/dev/null 2>&1 &
        sleep 2
